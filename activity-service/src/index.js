...
// รับ event จาก auth-service และ task-service (fire-and-forget)
...
// ── GET /api/activity/me ───────────────────────────────────────────────
// ดู activities ของตัวเอง (JWT required)
...
// ดู activities ทั้งหมด (admin only)
...
app.get('/api/activity/health', (_, res) =>
  res.json({ status: 'ok', service: 'activity-service', version: '1.0.0', time: new Date() })
);
...
async function start() {
  ...
  while (retries > 0) {
    try {
      ...
      // สร้าง table ถ้ายังไม่มี (Railway fallback)
      await pool.query(`
